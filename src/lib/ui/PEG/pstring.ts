...
export class PegString extends PegThing {
	...
	constructor(
		...
		// Trying to mimic default values
		this.mCursorPos = new PegPoint(0, 0);

		// TODO : change this ??
		let PEG_STRING_FONT = 1; // "SysFont"

		this._pText = new PegTextThing(
			text,
			wStyle & (TT_COPY | EF_EDIT),
			PEG_STRING_FONT
		);

		this.miMarkEnd = 0; // tstrlen(text);
		...
		switch (mesg.wType) {
			...
			case PegSystemMessage.PM_CURRENT:
				...
				if (this._pText.DataGet()) {
					...
					this.mpBackup = tstrcpy(this.mpBackup, this._pText.DataGet());
	...
	Draw(): void {
		...
		let point: PegPoint = new PegPoint(
			this.mClient.wTop,
		...
		if (this._pText.mpText) {
			if (this.state.mbMarked) {
				...
				this.DrawText(
					point,
					this._pText.mpText.slice(this.miFirstVisibleChar),
					color,
					this._pText.mpFont
				);
			}
	...
	DrawMarked(): void {
		...
		if (this.miMarkStart > this.miFirstVisibleChar) {
			pTemp = tstrcpy(pTemp, this._pText.mpText);
			pTemp[this.miMarkStart] = "\0".charCodeAt(0); // Sorry but JS is JS
			this.DrawText(
				point,
				pTemp.slice(this.miFirstVisibleChar),
				color,
				this._pText.mpFont
			);
		}
		...
		if (this.miMarkEnd > this.miFirstVisibleChar) {
			...
			if (this.miMarkStart >= this.miFirstVisibleChar) {
				pTemp = tstrcpy(pTemp, this._pText.mpText.slice(this.miMarkStart));
		...
		if (this.miMarkEnd < wLength) {
			pTemp = tstrcpy(pTemp, this._pText.mpText.slice(this.miMarkEnd));
