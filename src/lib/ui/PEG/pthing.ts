...
export abstract class PegThing {
	...
	constructor(p1: PegRect | WORD, p2: WORD, wStyle?: WORD) {
	...
	Message(mesg: PegMessage): SIGNED {
		switch (mesg.wType) {
			...
			case PegSystemMessage.PM_DESTROY:
				if (mesg.pSource) {
					let pkill: PegThing = mesg.pSource;
					this.MessageQueue().Purge(pkill);
					// delete pkill;
				}
				break;
			...
			default:
				if (this.mpParent) {
					if (this.mpParent != this.Presentation()) {
						return this.mpParent.Message(mesg);
					}
				}
				break;
		}
	...
	Add(what: PegThing, bDraw: boolean = true): void {
		...
		if (this.mpFirst) {
			...
			if (
				this.mpFirst.StatusIs(PSF_ALWAYS_ON_TOP) &&
				...
				if (
					!what.StatusIs(PSF_VIEWPORT) &&
					this.mpFirst.StatusIs(PSF_VIEWPORT)
				) {
					pTemp = this.mpFirst;
					while (pTemp.mpNext) {
