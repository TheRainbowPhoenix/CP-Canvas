...
import {
	VerdanaFont16_data_table,
	VerdanaFont16_offset_table,
} from "./native/fonts";
...
class ScribbleWindow extends CPModuleWindow {
	constructor(rect: PegRect, frame: CPMainFrame) {
...
function PegAppInitialize(pPresentation: PegPresentationManager) {
	...
	const rect: PegRect = PegRect.Set(
		MAINFRAME_LEFT,
		MAINFRAME_TOP,
		MAINFRAME_RIGHT,
		MAINFRAME_BOTTOM
...
export class TestViewScreen extends PegDecoratedWindow {
	...
	BuildScreen() {
		...
		let p_icon_text_2 = new PegString(
			childRect,
			_s("Insert Zip Disk"),
			0,
			FF_NONE | TT_COPY
...
export function peg_test(data) {
	...
	setTimeout(() => {
		...
		let newMessage: PegMessage = new PegMessage(PegSystemMessage.PM_EXIT);
		newMessage.pTarget = pPresentation;
		pMsgQueue.Push(newMessage);
	}, 6000);

	pPresentation.Execute().then((ret) => {
		console.log("!! PEG EXIT !! Code returned ", ret);
	});
}
